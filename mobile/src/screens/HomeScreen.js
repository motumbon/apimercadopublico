...
export default function HomeScreen({ navigation }) {
  ...
  // Últimas licitaciones con OC - ordenadas por fecha de última OC
  const ultimasConOC = licitaciones
    .filter(l => (ordenesPorLicitacion[l.codigo] || []).length > 0)
    .sort((a, b) => {
      const ordenesA = ordenesPorLicitacion[a.codigo] || [];
      const ordenesB = ordenesPorLicitacion[b.codigo] || [];
      
      // Obtener fecha más reciente de cada licitación
      const fechaMaxA = ordenesA.reduce((max, oc) => {
        const fecha = oc.fecha_envio || oc.fecha_aceptacion || '';
        return fecha > max ? fecha : max;
      }, '');
      
      const fechaMaxB = ordenesB.reduce((max, oc) => {
        const fecha = oc.fecha_envio || oc.fecha_aceptacion || '';
        return fecha > max ? fecha : max;
      }, '');
      
      return fechaMaxB.localeCompare(fechaMaxA); // Orden descendente (más reciente primero)
    })
    .slice(0, 5);
